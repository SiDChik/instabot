...
def load_uuid_and_cookie(self, load_uuid=True, load_cookie=True):
    ...
    with open(self.cookie_fname, "r") as f:
        data = json.load(f)
        load_uuid_and_cookie_dict(self, data, load_uuid=True, load_cookie=True)

    self.is_logged_in = True
    return True

def load_uuid_and_cookie_dict(self, data, load_uuid=True, load_cookie=True):
    if "cookie" in data:
...
def get_uuid_and_cookie_data(self):
    return {
        ...
        "timing_value": {
            ...
            "last_experiments": getattr(self, 'last_experiments', None),
        },
